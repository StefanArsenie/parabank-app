...
test.describe('Accounts Overview Page', () => {
    test('Display the account with valid balance', {tag: '@regression'}, async ({page, accountOverviewPage}) => {
        await test.step('Verify that page is displayed', async() => {
            await expect(accountOverviewPage.title).toHaveText(' Accounts Overview ')
        })
